...
export async function computeStatus(cfRay: string | null): Promise<StatusResponse> {
  ...
  const status: ComponentStatus = components.some((component) => component.status === 'outage')
    ? 'outage'
